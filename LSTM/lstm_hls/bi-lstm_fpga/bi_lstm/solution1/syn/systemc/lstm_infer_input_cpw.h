...
#ifndef _lstm_infer_input_cpw_H_
#define _lstm_infer_input_cpw_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "lstm_infer_input_cpw_memcore.h"

SC_MODULE(lstm_infer_input_cpw) {
...
protected:
    ...
    // buffer instances
    lstm_infer_input_cpw_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(lstm_infer_input_cpw) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "lstm_infer_input_cpw_memcore_U_";
            name += szTmp;

            buffer[i] = new lstm_infer_input_cpw_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "lstm_infer_input_cpw.trace");
            // ports
