...
void gemvm_quant(
    ...
    #pragma HLS ARRAY_PARTITION variable=a cyclic factor=16 dim=2  // ԭ16��8
    #pragma HLS ARRAY_PARTITION variable=b cyclic factor=16       // ԭ16��8
	for(int r=0; r<HIDDEN_DIM; ++r) {
        accum_t acc = 0;
        for(int c=0; c<INPUT_DIM + HIDDEN_DIM; ++c) {
            #pragma HLS PIPELINE II=2 // ԭII=2��2
            #pragma HLS UNROLL factor=16  // ԭ16��8
            acc += a[r][c] * b[c];
        }
        res[r] = acc;
    }
...
ap_uint<10> quantize_index(T x) {
    ...
    return ap_uint<10>(hls::floor(normalized + temp_t(0.5)));  // ��������
}
*/
 // �Ż���ļ����
